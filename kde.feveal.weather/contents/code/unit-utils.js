...
function refreshTooltipSubText() {
    // Función requerida por el proveedor pero no necesaria
}
...
function convertDate(date, timezoneType, offset) {
    // PROTECCIÓN: Si date es undefined, null, o no es válido
    if (!date) {
//        console.log("convertDate: date is undefined/null, using current date")
        date = new Date()
    }
    ...
    // Si aún no es un objeto Date válido, usar fecha actual
    if (!(date instanceof Date) || isNaN(date.getTime())) {
//        console.log("convertDate: invalid date, using current date")
        date = new Date()
    }
